...
const ItemsListBanner = ({
  ...
  const backgroundJumbotron = 
    isMobile && CategoryShop === 'men' ?  bannerCoverMenMobile :
    isMobile && CategoryShop === 'women' ?  bannerCoverWomenMobile :
    isMobile === false && CategoryShop === 'men' ? bannerCoverMenPc :
    bannerCoverWomenPc

  return (
    <Jumbotron style={backgroundJumbotron}>
      <Container style={textBanner}>
        <h1 className="display-3" style={titleH1Pc}>{CategoryShop === 'men' ? 'men' : 'women' }</h1>
