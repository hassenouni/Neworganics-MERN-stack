...
const decorationData = CategoryShop => CategoryShop === 'women' ? categoriesWomenData : categoriesMenData;

const eachCategory = (CategoryShop, oneKeywordForFilter) => (
  decorationData(CategoryShop).map(x=>(
    <Col md='4' style={styles.card} key={x.cardTitle}>
      <Card>
          <div style={styles.cardBtn} onClick={()=>oneKeywordForFilter(x.cardTitle)}>
            <Link to={`/productslist/${CategoryShop}/`}>
...
const ItemsListCategoryShopHomepage = props => {
  const { CategoryShop } = props.match.params
  const { oneKeywordForFilter } = props
  return (
  <Container style={{paddingTop:'30px', paddingBottom:'50px'}}>
    <Col md="12">
      <h1 style={{fontSize: '40px', textAlign:'center', padding:'20px'}}>{CategoryShop.charAt(0).toUpperCase()+CategoryShop.slice(1)} selection</h1>
      <Row>
        {eachCategory(CategoryShop, oneKeywordForFilter)}
      </Row>
    </Col> 
  </Container>
...
export default connect(mapStateToProps, mapDispatchToProps)(ItemsListCategoryShopHomepage);
